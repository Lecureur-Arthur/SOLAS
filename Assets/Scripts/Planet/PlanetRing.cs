...
public class PlanetRing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            float progess = (float)i / (float)segments;
            float angle = Mathf.Deg2Rad * progess * 360;
            ...
            uv[i * 2] = uv[i * 2 + halfway] = new Vector2(progess, 0f);
            uv[i * 2 + 1] = uv[i * 2 + 1 + halfway] = new Vector2(progess, 1f);
